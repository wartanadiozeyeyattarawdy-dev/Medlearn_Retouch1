-- ============================================================
-- FINAL - Toutes les tables pour MedLearn
-- ============================================================
...
CREATE POLICY "user read own game attempts" ON public.game_attempts
  FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY "user insert own game attempts" ON public.game_attempts
  FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());

CREATE INDEX idx_game_attempts_user ON public.game_attempts(user_id);
CREATE INDEX idx_game_attempts_module ON public.game_attempts(module_id);

-- 5. Ajouter youtube_url à lessons
ALTER TABLE public.lessons 
ADD COLUMN IF NOT EXISTS youtube_url TEXT;

-- 6. Vues pour statistiques
